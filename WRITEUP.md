...
**What** `Tmask` **is.** LLaDA-V is a masked *diffusion* LM, not an autoregressive one.
It does not emit tokens left to right. You hand it a sequence in which the answer
slots are already present as `[MASK]` placeholders, and it iteratively denoises them
into real tokens — every position at once, over several steps. This carries a
consequence with no LLaVA analogue: **the answer length is fixed before decoding
begins**, by how many `[MASK]` tokens you append. Here, k = 32.

```
[system prompt] [729 image tokens] [question] ASSISTANT: [MASK] x 32
                |<----- T2 ------>|                      |<- Tmask ->|
                |<------------- T3 ------------------->|
                 ^pos0 is here (position 0 of the full sequence)
```

`Tmask` is the last-layer hidden state at those 32 mask positions: `Tmask.avg` is the
mean over them, `Tmask.first` and `Tmask.last` the two end positions.

Why it matters: every LLaDA-V number before this one was **off-policy**. Clean mode
feeds the prompt with no mask span at all — structurally comparable to LLaVA, but a
configuration the model never runs at inference. `Tmask` is the state the decoder
actually reads while deciding what to say. To ask whether the model *has* the
information at the moment it commits to an answer, this is the only honest tap.

Note also what the diagram makes obvious: `T3` spans ~806 positions of which 729 are
image, which is why `T3.avg` tracks `T2.avg` closely, while `T3.pos0` and `T3.final`
are single *text* positions and behave completely differently.
